...
public class ShoppingCartServiceImpl implements ShoppingCartService {
    ...
    public String saveCartItem(ShoppingCartItem shoppingCartItem) {
        ...
        Product product = goodsMapper.selectByPrimaryKey(shoppingCartItem.getGoodsId());
    ...
    public List<ShoppingCartItemVO> getMyShoppingCartItems(Long userId) {
        ...
        if (!CollectionUtils.isEmpty(shoppingCartItems)) {
            ...
            if (!CollectionUtils.isEmpty(goods)) {
                goodsMap = goods.stream().collect(Collectors.toMap(Product::getGoodsId, Function.identity(), (entity1, entity2) -> entity1));
            }
            for (ShoppingCartItem shoppingCartItem : shoppingCartItems) {
                ...
                if (goodsMap.containsKey(shoppingCartItem.getGoodsId())) {
                    Product productTemp = goodsMap.get(shoppingCartItem.getGoodsId());
                    shoppingCartItemVO.setGoodsCoverImg(productTemp.getGoodsCoverImg());
                    String goodsName = productTemp.getGoodsName();
                    
                    if (goodsName.length() > 28) {
                        goodsName = goodsName.substring(0, 28) + "...";
                    }
                    shoppingCartItemVO.setGoodsName(goodsName);
                    shoppingCartItemVO.setSellingPrice(productTemp.getSellingPrice());
